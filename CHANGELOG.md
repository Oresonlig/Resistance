...
**BW-mätsätt (Chins m.fl.):** grafens Y-axel visade tidigare `extra+bodyweight` (samma kombinerade tal som avgör det officiella PR:et) — en kroppsviktsnedgång fick styrkan att se ut att sjunka trots oförändrad tillagd vikt. Ny `progressionSetValue(measure, set)` plottar nu bara `extra` för `bw`-mätsätt (matchar headline-siffran "BW + Xkg" i kortet), measure-drivet (`m.pr==='extra'`) — gäller alla bodyweight-övningar, inte bara Chins. Den officiella PR-badgen (`getAllPRs`/`prValue`) är oförändrad, fortsatt kombinerad. Niklas: "man ska inte straffas för att man deffar ner sig men bibehåller sin styrka." 174 tester gröna — canvas/SVG-rendering saknar testtäckning, kräver browser-verifiering.

---

## 3.78.5 — 2026-07-23
**HiDPI/retina-oskärpa på båda canvas-graferna (Niklas desktop-skärmdump, `Bugg/graphbugg1.png`).** Grid, linje och axel-text på PR-grafen (och Weight Curve) syntes suddiga medan all omkringliggande DOM-text var knivskarp — klassisk canvas-vs-CSS-upplösnings-mismatch. Rotorsak: `c.width=W` satte bildbufferten i CSS-pixlar, men `.pr-chart-canvas{width:100%!important}`/`canvas#weightChart{width:100%!important}` sträcker sedan canvasen till den FYSISKA pixel-bredden på skärmen (devicePixelRatio, ofta 2x på retina/desktop) — samma effekt som att zooma in en lågupplöst bild. All riktig DOM-text runtomkring renderas alltid i full skärmupplösning oavsett canvas-bufferten, därav den skarpa kontrasten som gjorde felet så uppenbart på desktop (mindre uppenbart men lika verkligt på mobilen — förklarar Niklas tidigare "svårläst"-kommentar om samma grafer). Fix (båda `drawPRChart` och `drawChart`): bufferten sätts nu i FYSISKA pixlar (`c.width=W*dpr;c.height=H*dpr`), CSS-höjden låses separat (`c.style.height=H+'px'`, bredden styrs redan av CSS), och `ctx.scale(dpr,dpr)` låter all befintlig rit-kod fortsätta jobba i CSS-pixel-koordinater helt oförändrad. 174 tester gröna (ingen testtäckning för canvas-rendering — ren visuell fix, kräver browser-verifiering).
...
## 3.77.0 — 2026-07-18
**Measure följer ÖVNINGEN vid swap (rotfix, samma bugklass som 3.76.0 men för mätsätt).** Niklas gym-rapport: 5 slumpade inswappade övningar visade alla BW-input à la Chins. Rotorsak: 3.76.0-fixen (delad swap-medveten resolver som kollar custom OCH bibliotek) applicerades bara på TAGG-läsarna — measure-sidans `effEx` fanns kvar i TRE duplicerade kopior (render, saveExercise, captureInputsToSetsBuffer) som alla bara kollade `customExercises` → inswappad BIBLIOTEKS-övning föll tillbaka till slottens ex och ärvde dess `bw`/`timed`-flaggor via `measureFromFlags`-fallbacken i getMeasure. Dessutom skickade `getSetTargets` och tag-editorn (Logged as-dropdown + sprint-base) raw slot-`ex` rakt in i getMeasure/getSprintBase. **Fix:** `_tagFlagSource` omdöpt till `getEffectiveEx` (samma semantik: extras → sig själv; swap → custom > bibliotek > slot) och ALLA sex measure/sprintBase-läsställen konverterade till den delade resolvern. De tre duplicerade effEx-beräkningarna raderade. +4 tester (bibliotek-swap i BW-slot → weight, explicit library-measure vinner, custom-flaggor vinner fortfarande, oswappad slot opåverkad). 167 tester gröna.

---

## 3.76.0 — 2026-07-14
**Taggar följer ÖVNINGEN, inte slotten (rotfix efter "Internal Rotator Cuff blev bodyweight").** Niklas-rapport: swappad övning bytte plötsligt input-klassificering. Rotorsak (två delar av samma arkitekturfel): (1) bas-flaggor (ramp/bw/uni/timed/singles) lästes från SLOT-objektet — `effEx = swappedCustom || ex` kollade bara customExercises, så en inswappad BIBLIOTEK-övning ärvde alltid den bortswappade basövningens flaggor (A1/D1/E1 ramp, F1 bw — Niklas hade manuellt neutraliserat alla fyra med false-overrides utan att veta varför det behövdes); (2) `exerciseTagOverrides` nycklades på slot-id → en override satt för övning X spökade kvar på slotten och drabbade nästa inswappade övning (måndags-incidenten), medan `exerciseMeasureOverride` redan var korrekt exId-nycklad — därav asymmetrin bw-hint/reps-chip. **Fix:** EN delad swap-medveten resolver (`getTagActive`/`_tagFlagSource`/`tagOverrideKeyFor`): override (kanoniskt exId) > inswappad övnings flaggor (custom ELLER bibliotek) > slot-flaggor. Alla fem läsare konverterade (render, tag-editor, toggleExTag, ensureExtraSets, getPrimaryTag→getSetTargets/hints); toggleExTag skriver nu på kanoniskt exId. Extras behåller session-scopat extra_-id (städning + saveExtraToProgram-promovering intakt, promoveringen skriver nu till kanoniskt id). **Engångsmigrering** `migrateTagOverridesToExId` (flag `tagOverridesExId_v1`): rekeyar slot→exId via slotKeyToExId (utan draft-temp-swaps) + getRenderExId för added_*; per-nyckel-merge där kanonisk vinner; okända nycklar lämnas. Idempotent `rekeyTagOverridesToExId` körs även efter BÅDA template-import-vägarna (pre-3.76-filer har slot-nycklar). +4 tester (arv-fix, ingen-swap-oförändrat, kanonisk skrivning + cross-slot-delning, migrering) & 2 uppdaterade till kanoniska nycklar. 163 tester gröna.
...
## 3.72.6 — 2026-07-09
**Forced Rest days visas nu i Progress → History (med datum), så Niklas kan se om han faktiskt vilar.** Niklas hittade på "Forced Rest" just för att han är dålig på att vila — men märker att han fuskar även med dem, och ville kunna verifiera att han faktiskt följer dem. Rotläge: en avklarad Forced Rest sparades redan i `cycle.done[passId]={date,exercises:[]}` (via `doneRest`) MEN pushades aldrig till `state.log` → History (som bara läser `state.log`) visade dem aldrig. Fix (DISPLAY-ONLY, medvetet): rest-dagar hålls kvar UTANFÖR `state.log` så PR-logiken och "Sessions"-räknaren är orörda (Niklas: "inte på PR såklart, bara att de ligger där bland passen, vilket datum"). `renderProgress` bygger nu en kombinerad History-lista: sessioner (från `state.log`) + avklarade rest-dagar (insamlade från ALLA rundors `cycle.done`-mappar), sorterade tillsammans på timestamp. `doneRest` sätter nu även `timestamp` + `rest:true` på snapshotet (sorterbart + entydigt); gamla rest-dagar utan timestamp får en best-effort datumparsning. Rest-raden renderas distinkt: guld vänsterkant + "✓ Rest"-badge (som kedjans gyllene vilodagar), ingen body/toggle/copy — bara ett daterat kvitto. 152 tester gröna. **EJ browser-verifierat** (sandbox saknar browser).

---

## 3.72.5 — 2026-07-09
**Login: font-regression fixad (kontroller tema-oberoende) + mobil fokuskedja mycket mindre & flyttad åt höger (Niklas, forts. från igår).** **(1) REGRESSION — login-kontrollernas typsnitt temades.** 3.72.3 satte `--font-display: var(--display)` för att tema wordmarken, men det themade OCKSÅ auth-kontrollerna → "Continue with Google" (+ Log In / Sign Up / reset-titel) bytte typsnitt OCH storlek per tema (serif i Cosmic, sans i Ember/NC) och hoppade/resiz:ade. Niklas krav (upprepat, jfr 3.69.0 auth-inputs): login-MÖJLIGHETERNA ska vara opåverkade av temabyte; bara branding/scen/glöd/hex ska påverkas. Fix: ny `--font-brand: var(--display)` som ENBART wordmarken (`.cl-mark`) använder; `--font-display` tillbaka till FAST Archivo för alla kontroller. Knapp-FÄRG fortsätter temas (det gillar han). **(2) Mobil fokuskedja.** Låg centrerad (`left:50%`) i nästan full storlek → glödheta länken låg rakt över Google-knappen och blockerade login. Nu speglar den desktop: undanskuffad åt HÖGER (`left:76%`) och nedskalad MYCKET (0.56) → liten forge-accent i övre högra hero-bandet, fri från formuläret. Bara glödheta länken behåller etiketten "+ FORGE YOUR NEXT LINK", som nu sträcker sig åt VÄNSTER från kedjan (desktop-beteende). Skalningen ligger på SVG:n (inte host) så etiketten (syskon till SVG:n) förblir full storlek + JS-positionerad mot den oskalade host:en. **EJ browser-verifierat** (sandbox saknar browser) — pushas live per Niklas instruktion, mobil-layouten behöver hans ögon.
